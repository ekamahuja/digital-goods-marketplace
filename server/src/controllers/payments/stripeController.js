...
export const processSession = async (req, res, next) => {
  try {
    ...
    const userAgent = req.headers["user-agent"] || "Could not grab user agent";
    ...
    const generatedKeys = await generateKeys(
      pData.keyPrefix,
      pData.keyType,
      pData.keyQuantity * stripeSession.metadata.quantity
    );
